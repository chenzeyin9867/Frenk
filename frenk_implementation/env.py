...
class PassiveHapticsEnv(object, radius=0.5, random=False):
    ...
    def calculatePath():
        curDirection = np.array([np.cos(self.v_direction), np.sin(self.v_direction)])
...
class Steinickef_Redirector(object):
    ...
    MOVEMENT_THRESHOLD = 0.2f # meters per second
    ROTATION_THRESHOLD = 1.5f # degrees per second
